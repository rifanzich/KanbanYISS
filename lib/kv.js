import { getRedisClient } from "./redisClient";

const ACCOUNT_PREFIX = "account:";

async function scanKeys(client, pattern) {
  const keys = [];
  if (typeof client.scanIterator === "function") {
    for await (const key of client.scanIterator({ MATCH: pattern, COUNT: 100 })) {
      keys.push(key);
    }
  } else {
    let cursor = 0;
    do {
      const reply = await client.scan(cursor, { MATCH: pattern, COUNT: 100 });
      cursor = typeof reply === "number" ? reply : (reply && reply.cursor) || 0;
      const found = Array.isArray(reply) ? reply[1] : (reply && reply.keys) || [];
      if (found && found.length) keys.push(...found);
    } while (cursor !== 0 && cursor !== "0");
  }
  return keys;
}

export function accountKey(username) {
  return `${ACCOUNT_PREFIX}${username}`;
}

export async function listAccounts() {
  const client = await getRedisClient();
  const keys = await scanKeys(client, `${ACCOUNT_PREFIX}*`);
  if (!keys.length) return [];
  const raws = await Promise.all(keys.map((k) => client.get(k)));
  return raws
    .filter(Boolean)
    .map((r) => JSON.parse(r))
    .map((a) => ({ username: a.username, role: a.role }));
}

export async function getAccount(username) {
  const client = await getRedisClient();
  const raw = await client.get(accountKey(username));
  return raw ? JSON.parse(raw) : null;
}

export async function saveAccount(account) {
  const client = await getRedisClient();
  await client.set(accountKey(account.username), JSON.stringify(account));
}

export async function deleteAccount(username) {
  const client = await getRedisClient();
  await client.del(accountKey(username));
}

// Renames an account's record in-place (same role/passwordHash, new key).
export async function renameAccount(oldUsername, newUsername) {
  const client = await getRedisClient();
  const raw = await client.get(accountKey(oldUsername));
  if (!raw) return false;
  const account = JSON.parse(raw);
  account.username = newUsername;
  await client.set(accountKey(newUsername), JSON.stringify(account));
  await client.del(accountKey(oldUsername));
  return true;
}

// Moves every personal:<oldUsername>:* key (that user's boards/notes/indexes)
// over to personal:<newUsername>:* so a rename doesn't orphan their data.
export async function migratePersonalKeys(oldUsername, newUsername) {
  const client = await getRedisClient();
  const oldPrefix = `personal:${oldUsername}:`;
  const newPrefix = `personal:${newUsername}:`;
  const keys = await scanKeys(client, `${oldPrefix}*`);
  for (const key of keys) {
    const suffix = key.slice(oldPrefix.length);
    await client.rename(key, `${newPrefix}${suffix}`);
  }
}

// Swaps the username inside every shared ("Tim") workspace's member roster
// so a renamed member keeps their access to team workspaces.
export async function renameInSharedRoster(oldUsername, newUsername) {
  const client = await getRedisClient();
  const key = "shared:ruang-shared-index";
  const raw = await client.get(key);
  if (!raw) return;
  let list;
  try {
    list = JSON.parse(raw);
  } catch (e) {
    return;
  }
  if (!Array.isArray(list)) return;
  const updated = list.map((ws) =>
    Array.isArray(ws.allowedMembers)
      ? { ...ws, allowedMembers: ws.allowedMembers.map((m) => (m === oldUsername ? newUsername : m)) }
      : ws
  );
  await client.set(key, JSON.stringify(updated));
}

export async function countAccounts() {
  const client = await getRedisClient();
  const keys = await scanKeys(client, `${ACCOUNT_PREFIX}*`);
  return keys.length;
}

// ---- Generic workspace key/value storage, namespaced per user or shared ----
// Values here are already JSON-stringified by the app before they arrive, so
// they're stored and returned as raw strings (no extra JSON encode/decode).
function namespacedKey(rawKey, shared, username) {
  return shared ? `shared:${rawKey}` : `personal:${username}:${rawKey}`;
}

export async function storageGet(rawKey, shared, username) {
  const client = await getRedisClient();
  return client.get(namespacedKey(rawKey, shared, username));
}

export async function storageSet(rawKey, value, shared, username) {
  const client = await getRedisClient();
  const toStore = typeof value === "string" ? value : JSON.stringify(value);
  await client.set(namespacedKey(rawKey, shared, username), toStore);
}

export async function storageDelete(rawKey, shared, username) {
  const client = await getRedisClient();
  await client.del(namespacedKey(rawKey, shared, username));
}

export async function storageListKeys(prefix, shared, username) {
  const client = await getRedisClient();
  const nsPrefix = namespacedKey(prefix || "", shared, username);
  const keys = await scanKeys(client, `${nsPrefix}*`);
  const stripLen = namespacedKey("", shared, username).length;
  return keys.map((k) => k.slice(stripLen));
}

const RUANG_DATA_PREFIX = "ruang-data-";

// Team ("Tim") workspace content lives under shared:ruang-data-<id>. Only the
// workspace's approved roster (allowedMembers) or an admin may read/write it.
// Personal-mode keys are already isolated per-user by namespacedKey() above,
// so this only needs to gate the shared case.
export async function canAccessSharedKey(rawKey, user, action = "read") {
  if (!user) return false;
  if (user.role === "admin") return true;

  if (rawKey === "ruang-shared-index") {
    // Reading the index is allowed so members can see workspaces they are part of
    if (action === "read") return true;
    // Deleting the whole shared index is strictly admin-only
    if (action === "delete") return false;
    // Writing/updating workspace roster/index is permitted for authenticated users
    return true;
  }

  if (rawKey.startsWith(RUANG_DATA_PREFIX)) {
    const workspaceId = rawKey.slice(RUANG_DATA_PREFIX.length);
    const client = await getRedisClient();
    const raw = await client.get(namespacedKey("ruang-shared-index", true, user.username));
    let list = [];
    try {
      list = raw ? JSON.parse(raw) : [];
    } catch (e) {
      list = [];
    }
    const ws = Array.isArray(list) ? list.find((w) => w.id === workspaceId) : null;
    if (!ws) {
      // Fail-close: workspace is not registered in shared index
      return false;
    }
    const allowed = ws.allowedMembers || [];
    return allowed.includes(user.username);
  }

  // Fail-close policy for any other key
  return false;
}
